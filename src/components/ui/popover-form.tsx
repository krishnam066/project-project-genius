  ...
  return (
    ...
    >
      <motion.button
        ...
        className="flex h-12 items-center border bg-gradient-to-r from-blue-500 to-purple-600 text-white px-6 text-sm font-medium outline-none hover:from-blue-600 hover:to-purple-700 transition-all duration-300"
      >
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            layoutId={`${title}-wrapper`}
            className="absolute p-1 overflow-hidden bg-background border shadow-2xl outline-none z-50"
            ref={ref}
            ...
            {showCloseButton && (
              <div className="absolute -top-[5px] left-1/2 transform -translate-x-1/2 w-[12px] h-[26px] flex items-center justify-center z-20">
                <button
                  ...
                  aria-label="Close"
                >
                  <ArrowUp className="text-muted-foreground/80" />
                </button>
              </div>
            )}
...
export function PopoverFormButton({
  ...
  return (
    <button
      type="submit"
      className="ml-auto flex h-8 w-26 items-center justify-center overflow-hidden rounded-md bg-gradient-to-b from-primary/90 to-primary px-4 text-xs font-semibold text-primary-foreground shadow-lg hover:shadow-xl transition-all duration-300"
    >
